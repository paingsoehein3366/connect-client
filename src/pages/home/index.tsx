...
export const Home = () => {
  const [onChangeData, setChangeData] = useState<string>()
  ...
  return (
    <div className="bg-bg">
      <div className="flex mt-14">
        <div className="flex flex-col items-center p-10 w-[50%] justify-center">
          <div>
          ...
          <div>
            <PillsInput
              variant="filled"
              rightSection={<CiSearch className="cursor-pointer text-blue-500" onClick={onSubmit} />}
              size="md"
            >
              <Pill.Group className="cursor-pointer">
                <Text>目的は?</Text>
                <Pill onClick={() => setChangeData('地域')}>地域</Pill>
                <Pill onClick={() => setChangeData('留学')}>留学</Pill>
                <Pill onClick={() => setChangeData('就職')}>就職</Pill>
                <PillsInput.Field placeholder="入力して下さい" onChange={(e) => setChangeData(e.target.value)} value={onChangeData} />
              </Pill.Group>
            </PillsInput>
          </div>
        </div>
      ...
      <div className="my-10">
        <Title order={3} className="text-center">おすすめ高級賃貸マンション</Title>
